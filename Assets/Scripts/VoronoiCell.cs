...
public class VoronoiCell
{
    ...
    public List<int> triangles_out;
    public List<int> triangles_in;
    ...
    public VoronoiCell(Vector3 seedPos, List<Vector3> points)
    {
        ...
        triangles_out = new List<int>();
        triangles_in = new List<int>();
        ...
        calc.GenerateHull(points, true, ref vertices, ref triangles_in, ref normals);
    }
    ...
    public void CutWithPlane(Vector3 p1, Vector3 p2, Vector3 p3)
    {
        ...
        var newTriangles_in = new List<int>();
        var newTriangles_out = new List<int>();
        var newNormals = new List<Vector3>();

        var planeTriangles = new List<int>();
        var planeVertices = new List<Vector3>();

        for (int i = 0; i < triangles_in.Count; i += 3)
        {
            var v1 = vertices[triangles_in[i]];
            var v2 = vertices[triangles_in[i + 1]];
            var v3 = vertices[triangles_in[i + 2]];
            ...
            {
                newVertices.Add(v1);
                newNormals.Add(normals[triangles_in[i]]);
                newTriangles_in.Add(newVertices.Count - 1);

                newVertices.Add(v2);
                newNormals.Add(normals[triangles_in[i + 1]]);
                newTriangles_in.Add(newVertices.Count - 1);

                newVertices.Add(v3);
                newNormals.Add(normals[triangles_in[i + 2]]);
                newTriangles_in.Add(newVertices.Count - 1);
                if (v1On && v2On)
                {
                ...
                (v1Above || v1On) && (!v2Above && !v2On) && ((v3Above || v3On)))
            {
                ...
                {
                    var temp = v1;
                    var tempAbove = v1Above;
                    var tempOn = v1On;
                    v1 = v3; v3 = v2; v2 = temp;
                    v1Above = v3Above; v3Above = v2Above; v2Above = tempAbove;
                    v1On = v3On; v3On = v2On; v2On = tempOn;
                }
                else if (!v2Above && !v2On)
                {
                    ...
                    var tempOn = v1On;
                    v1 = v2; v2 = v3; v3 = temp;
                    v1Above = v2Above; v2Above = v3Above; v3Above = tempAbove;
                    v1On = v2On; v2On = v3On; v3On = tempOn;
                }
                ...
                planeTriangles.Add(planeVertices.Count - 1);
        ...
        for (int i = 0; i < triangles_out.Count; i += 3)
        {
            var v1 = vertices[triangles_out[i]];
            var v2 = vertices[triangles_out[i + 1]];
            var v3 = vertices[triangles_out[i + 2]];
            bool v1Above = IsAbovePlane(v1, p1, p2, p3);
            bool v2Above = IsAbovePlane(v2, p1, p2, p3);
            bool v3Above = IsAbovePlane(v3, p1, p2, p3);
            bool v1On = IsOnPlane(v1, p1, p2, p3);
            ...
            {
                newVertices.Add(v1);
                newNormals.Add(normals[triangles_out[i]]);
                newTriangles_out.Add(newVertices.Count - 1);

                newVertices.Add(v2);
                newNormals.Add(normals[triangles_out[i + 1]]);
                newTriangles_out.Add(newVertices.Count - 1);

                newVertices.Add(v3);
                newNormals.Add(normals[triangles_out[i + 2]]);
                newTriangles_out.Add(newVertices.Count - 1);
                if (v1On && v2On)
                {
                    planeVertices.Add(v1);
        ...
        for (int i = 0; i < planeTriangles.Count; i += 2)
        {
            ...
            newTriangles_out.Add(cnt - 3);
            newTriangles_out.Add(cnt - 2);
            newTriangles_out.Add(cnt - 1);
        }

        vertices = newVertices;
        triangles_in = newTriangles_in;
        triangles_out = newTriangles_out;
        normals = newNormals;
    }
